...
class PortfolioService {
  constructor(db) {
    this.createPortfolio = (req, res, next) =>
    ...
    this.db = db;
    this.historicPortfolioService =
      new historic_portfolio_service_1.HistoricPortfolioService();
  }
  init() {
  ...
  getPortfolios(userId) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      for (var portfolio of result) {
        portfolio[api_definition_1.PayloadKey.POSITIONS] =
          yield this.getPositionsWithValuesById(userId, portfolio["id"]);
        portfolio[api_definition_1.PayloadKey.CURRENT_VALUE] =
          yield this.calculateTotalValue(
            portfolio[api_definition_1.PayloadKey.POSITIONS]
          );
        portfolio[api_definition_1.PayloadKey.HISTORIC_VALUE] =
          yield this.historicPortfolioService.calculateHistoricValue(
            portfolio[api_definition_1.PayloadKey.POSITIONS]
          );
  ...
  getPortfolioById(userId, portfolioId) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      result[api_definition_1.PayloadKey.POSITIONS] =
        yield this.getPositionsWithValuesById(userId, result["id"]);
      result[api_definition_1.PayloadKey.CURRENT_VALUE] =
        yield this.calculateTotalValue(
          result[api_definition_1.PayloadKey.POSITIONS]
        );
      result[api_definition_1.PayloadKey.HISTORIC_VALUE] =
        yield this.historicPortfolioService.calculateHistoricValue(
          result[api_definition_1.PayloadKey.POSITIONS]
        );
      return result;
  ...
  getPositionsWithValuesById(userId, portfolioId) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      for (const position of positions) {
        position[api_definition_1.PayloadKey.VALUES] = yield this.db.getValues(
          userId,
          portfolioId,
          position["id"]
        );
  ...
  calculateTotalValue(positions) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      if (maxDate.getTime() === this.getMaxDate().getTime()) {
        totalValueObject[api_definition_1.PayloadKey.MIN_DATE] =
          this.toIsoDateString(new Date());
        totalValueObject[api_definition_1.PayloadKey.MAX_DATE] =
          this.toIsoDateString(new Date());
      } else {
        totalValueObject[api_definition_1.PayloadKey.MIN_DATE] =
          this.toIsoDateString(minDate);
        totalValueObject[api_definition_1.PayloadKey.MAX_DATE] =
          this.toIsoDateString(maxDate);
      }
