...
class PortfolioService {
  ...
  async getPortfolios(userId: string) {
    ...
    for (var portfolio of result) {
      portfolio[PayloadKey.POSITIONS] = await this.getPositionsWithValuesById(
        userId,
        portfolio["id"]
      );
      portfolio[PayloadKey.CURRENT_VALUE] = await this.calculateTotalValue(
        portfolio[PayloadKey.POSITIONS]
      );
      portfolio[PayloadKey.HISTORIC_VALUE] =
        await this.historicPortfolioService.calculateHistoricValue(
          portfolio[PayloadKey.POSITIONS]
        );
  ...
  async getPortfolioById(userId: string, portfolioId: string) {
    ...
    result[PayloadKey.POSITIONS] = await this.getPositionsWithValuesById(
      userId,
      result["id"]
    );
    result[PayloadKey.CURRENT_VALUE] = await this.calculateTotalValue(
      result[PayloadKey.POSITIONS]
    );
    result[PayloadKey.HISTORIC_VALUE] =
      await this.historicPortfolioService.calculateHistoricValue(
        result[PayloadKey.POSITIONS]
      );
